...
class Curse():
    ...
    def dispdown(self, screen):
        ...
        self.action_menu.addSubEntry(MenuEntry('Down All', 5, action=self.downAll))
        ...
        if url:
            try:
                ...
                while p.poll() is None:
                    ...
                    self.scr.addstr(6+y, 3+x+l, li.split('.')[-1])
                    self.scr.refresh()
            except:
                p.kill()
    ...
    def changevalue(self):
        ...
        (y, x, l) = self.visible.posend
        width = self.scr.getmaxyx()[1]-32
        self.scr.addstr(6+y, x+2, ' '*width)
        curses.curs_set(1)
        curses.nocbreak()
        curses.echo()
        self.scr.move(6+y, x+2)
        name = self.scr.getstr()
        curses.noecho()
        curses.curs_set(0)
        curses.cbreak()
    ...
    def savesetting(self):
        ...
        #self.tabs.colseActiveTab()

    def dispsetting(self, screen):
        """ display settings """
        self.action_menu.removeAllEntries()
        self.action_menu.addSubEntry(MenuEntry('Change value', 7, \
                action=self.changevalue))
        self.action_menu.addSubEntry(MenuEntry('Save', action=self.savesetting))
        self.action_menu.addSubEntry(MenuEntry('Close', \
                action=self.tabs.closeActiveTab))

        self.listkeys = self.options.conf.keys()
        listvalues = []
        for key in self.listkeys:
            listvalues.append(self.options.conf[key])
        self.visible2 = List(screen, self.listkeys, False)
        self.visible2.display(10, 4, 15)
        self.visible = List(screen, listvalues, True)
        self.visible.display(28, 4, 15)

    def setting(self):
        """ settings """
        settings_tab = TabEntry('Settings', self.dispsetting)
